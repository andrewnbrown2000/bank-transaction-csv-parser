...
def trim_transaction(transaction)
  ...
  if transaction.is_a?(Withdrawal)
    transaction.vendor = transaction.vendor.split('   ')[0] if transaction.vendor.match(/\s{3}/)
    # transaction.vendor = transaction.vendor.split(/([0-9]+ )(([A-Za-z]+  ?)+)/)[0] if transaction.vendor.match(/([0-9]+ )(([A-Za-z]+  ?)+)/)
    transaction.vendor = transaction.vendor.match(/([0-9]+ )(([A-Za-z]+  ?)+)/)[2] if transaction.vendor.match?(/([0-9]+ )(([A-Za-z]+  ?)+)/)
